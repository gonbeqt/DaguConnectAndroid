...
class RegisterViewModel(private val apiService: ApiService):ViewModel() {
    ...
    fun register(firstName:String, lastName: String, username: String,  age: Int, email:String, isClient: Boolean, password:String, confirmPassword:String) {
        viewModelScope.launch { // Launch a coroutine to make the API call
            _registerState.value = RegisterState.Loading
            try {
                val response = apiService.register(RegisterRequest(firstName, lastName, username,  age, email, isClient, password, confirmPassword)) // API call
